...
function registerUser (req, res) {
    ...
    User.findOne({username}, (err, user) =>{
        ...
        if(user)
            res.status(400).json({message: {msgBody : 'username is already used', msgError: true}});
        else {
        ...
        if(err)
            ...
            req.user.save(err => {
                if(err)
                    ...
                    res.status(200).json({message : {msgBody : 'successfully created receipe', msgError : false}});
            });
        };
    });
};
...
function getUsersReceipe (req, res) {
    User.findById({_id : req.user._id}).populate('receipes').exec((err, document) => {
        if(err)
            ...
            res.status(200).json({receipes : document.receipes, authenticated : true});
    });
};
